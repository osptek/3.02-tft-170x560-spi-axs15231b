...
////////// LCD init code ///////////////////
//AGQ_V3.5_RRJ_R302103_3.02_231B_SPI_V4.0(VSP??).txt 
...
void DispFrame(void)
{
	...
	{
		WriteData(RED>>8);WriteData(RED);
		for(i=0;i<COL-4;i++){WriteData(BLACK>>8);WriteData(BLACK);}
		WriteData(RED>>8);WriteData(RED);
		WriteData(RED>>8);WriteData(RED);
		WriteData(RED>>8);WriteData(RED);
	}
	...
	WriteData(RED>>8);WriteData(RED);
	for(i=0;i<COL-4;i++){WriteData(0XFFFF);WriteData(0XFFFF);}
	WriteData(RED>>8);WriteData(RED);
	WriteData(RED>>8);WriteData(RED);
	WriteData(RED>>8);WriteData(RED);
...
void DispGrayHor8(void)	 
{
	...
	for(i=0;i<ROW;i++)
	{
		...
		for(j=0;j<8;j++)
		{
	        for(k=0;k<COL/8;k++)
			{		
				SendDataSPI(((j*4)<<3)|((j*8)>>3));		 
				SendDataSPI(((j*8)<<5)|(j*4));
